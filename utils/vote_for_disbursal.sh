...
if [ -z "$CANISTER_ARG" ]; then
  ...
  TOPIC=$(echo "$CANISTER_ARG" | didc decode --defs ../src/codelta_backend/codelta_backend.did --types '(Topic)')
  echo $TOPIC
fi
echo
...
echo
dfx canister call codelta_backend check_balance "$TOPIC" --network=$NETWORK
echo
